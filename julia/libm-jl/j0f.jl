...
function pzerof(x::Float32)
    ...
    z = 1f0 / (x * x)
    r = p[1] + z * (p[2] + z * (p[3] + z*(p[4] + z * (p[5] + z * p[6]))))
    s = 1f0 + z * (q[1] + z * (q[2] + z * (q[3] + z * (q[4] + z * q[5]))))
    1f0 + r / s
end
...
function ieee754_y0f(x::Float32)
    ...
    if ix <= 0x39000000 # x < 2^-13
        return u00 + tpi * log_unchecked(x)
    end
    z = x * x
    u = u00 + z * (u01 + z * (u02 + z * (u03 + z * (u04 + z * (u05 + z * u06)))))
    v = 1f0 + z * (v01 + z * (v02 + z * (v03 + z * v04)))
    u / v + tpi * (ieee754_j0f(x) * log_unchecked(x))
end
end
